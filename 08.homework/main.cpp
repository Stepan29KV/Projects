...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    std::thread thread_1{count_words, std::ref(input), std::ref(freq_dict)};
    if (thread_1.joinable())
      thread_1.join();
  }
  auto thread_2 = std::async(std::launch::async, print_topk, std::ref(std::cout), std::cref(freq_dict), std::cref(TOPK));
  thread_2.get();
